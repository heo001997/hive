...
export const DEFAULT_AUTO_RESOLVE_CONFLICT_PROMPT = `PR #{prNumber} can't be merged into {baseBranch} — the branch {featureBranch} has conflicts with {baseBranch}. Resolve the conflict end-to-end yourself, right here in this worktree, then push so Tu can review before merging.
...
5. Finish it: once everything builds and conflicts are gone, commit and push (git push). Do NOT merge the PR — stop here and tell Tu the branch is pushed and ready for his final review and merge.
