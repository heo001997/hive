 ...
 * `useBoardChatStore.createSelected` so the Speckit review-GATE can reuse the
...
export interface CreateTicketsFromDraftsOptions {
   ...
   * Speckit gate config for the review draft, `null` for the rest. When it
  ...
  /**
   * Force this session mode on every created ticket. Speckit chains need
   * `'build'` so their review gates can arm (`armSettleTimers`/`passesSettleGuards`
